...
def create_network_graph(context_freq, target_word, top_n=20):
    ...
    fig_network = go.Figure(data=[edge_trace, node_trace], layout=go.Layout(showlegend=False, hovermode='closest',
                            xaxis=dict(showgrid=False, zeroline=False, showticklabels=False),
                            yaxis=dict(showgrid=False, zeroline=False, showticklabels=False), margin=dict(b=20, t=20, l=300, r=20),
...
app.layout = html.Div([
    ...
    dcc.Input(id='target-word-input', type='text', value='빅데이터', placeholder='Enter a target word...'),
...
def update_figure(n_clicks, selected_scores, select_sentiment, fileter_col, noun_counts_max, dropdown_option, column_list):
    ...
    table_df.to_csv('./data/now_view.csv', index=False, sep='\t')
...
def update_graph(n_clicks, target_word):
    ...
    fig_network = create_network_graph(context_freq, target_word)
    return fig_network
...
def download_dataframe(n_clicks2):
    if n_clicks2 is None:
        ...
        return dcc.send_file('./data/now_view.csv')
